// Full stadium 3D scene — a faithful plain-JS port of the web app's
// web/src/games/karirs/render3d.ts (same constants, same rig/crowd/city
// construction, same animation math), converted since this vscode client
// has no bundler-friendly TypeScript pipeline and no separate render.ts
// 2D module to import FINISH_LINE/RACER_COLORS from (inlined below
// instead). Kept in lockstep with the web version deliberately — port
// changes there over here too, rather than letting the two drift.
...
// The torso's own Y position within the rig group.
const TORSO_Y = 0.82;
...
const facePendingCallbacks = new Map();

function loadFaceTexture(url, onReady) {
...
function makeSkyTexture() {
  ...
  function paintCloud(cx, cy, scale) {
    const puffs = [
      [0, 0, 1],
...
function makeNameSprite(name, isMine) {
...
function buildRacerRig(color, gaitSeed, faceImageUrl) {
  ...
  if (faceImageUrl) {
    loadFaceTexture(faceImageUrl, (tex) => {
      faceDecal.material.map = tex;
      faceDecal.material.needsUpdate = true;
...
export function createKarirsScene3D(canvas) {
  ...
  // race reads left-to-right on screen — lanes fan out in Z (toward/away
  // from camera) instead.
  ...
  const phaseAt = (x, z) => ((Math.sin(x * 12.9898 + z * 78.233) * 43758.5453) % 1) * Math.PI * 2;
  ...
  const crowdBuckets = CROWD_COLORS.map((color) => ({
    slots: [],
  ...
  function updateCrowd(t, excited) {
  ...
  function placeBuilding(x, z, seed) {
  ...
  function paintLaneDashes(alongMin, alongMax, fixedCoord, alongIsX) {
  ...
  let focusedRacerName = null;

  let lastWidth = 0;
  let lastHeight = 0;

  const faceRenderers = new Map();
  const faceCamLookAt = new THREE.Vector3();

  function setFaceCanvases(canvases) {
    ...
    for (const [name, canvas2] of canvases) {
      ...
      const camera2 = new THREE.PerspectiveCamera(40, 1, 0.05, 20);
      faceRenderers.set(name, { canvas: canvas2, renderer: renderer2, camera: camera2, lastWidth: 0, lastHeight: 0 });
    }
  }
  ...
  function rebuildRacers(racerNames, myBetRacerName, faceImageUrls, signatureMoves) {
    ...
    while (laneDividers.children.length) {
      const child = laneDividers.children[0];
  ...
  function update({ racerNames, playback, winningName, isResolved, myBetRacerName, faceImageUrls, signatureMoves }) {
    ...
    for (const name of racerNames) {
      ...
      if (bubble) {
        ...
        bubble.material.opacity = nb * BUBBLE_MAX_OPACITY;
      }
    }
  ...
  function dispose() {
    ...
    for (const child of laneDividers.children) disposeMesh(child);
    disposeMesh(floor);
