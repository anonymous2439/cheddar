...
// simple hand-rolled dynamics — the particle counts here are small
// enough that this is nowhere near a bottleneck.
//
// Ported from games/luba/client/src/effects.js.
...
const SPRAY_GRAVITY = 14; // its own constant, not physics.ts's GRAVITY — tuned for how the spray reads

export interface BloodSprayHandle {
...
// its reference.
export function spawnBloodSpray(scene: THREE.Scene, x: number, y: number, z: number): BloodSprayHandle {
  const positions = new Float32Array(PARTICLE_COUNT * 3);
  const velocities: { x: number; y: number; z: number }[] = new Array(PARTICLE_COUNT);
...
export function spawnDashEffect(scene: THREE.Scene, x: number, y: number, z: number, dirX: number, dirZ: number): BloodSprayHandle {
  const positions = new Float32Array(DASH_PARTICLE_COUNT * 3);
  const velocities: { x: number; y: number; z: number }[] = new Array(DASH_PARTICLE_COUNT);
...
export function spawnSmokeCloud(scene: THREE.Scene, x: number, y: number, z: number, radius: number, durationMs: number): BloodSprayHandle {
